...
def test_run_sync_pipeline_success(
    ...
    assert result["filename"] == "upload_2026-04-12-10-00-00.csv"
...
def test_databricks_target_methods_execute_queries():
    ...
    with mock.patch(
        ...
        assert target.upload_to_databricks("/tmp/file.csv", "file.csv") == [
            ["ok"]
        ]
        assert target.upsert_new_data("file.csv") == [["ok"]]
...
def test_extract_states_to_csv_without_hot_copy(tmp_path):
    ...
    out_csv = tmp_path / "out_no_copy.csv"
